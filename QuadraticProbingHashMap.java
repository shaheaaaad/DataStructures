...
public class QuadraticProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if (key == null || value == null) {
            throw new IllegalArgumentException("The key or value inputted is NULL!");
        }
        if ((size + 1.0) / table.length > MAX_LOAD_FACTOR) { /// adding -
        ...
        // start probing
        int counter = table.length / 2;
        int n = 0;
        int fdi = -1;

        while (table[i] != null && counter != 0) {

            if (table[i].isRemoved() && fdi == -1) {
                fdi = i;
            }
            ...
            counter--;
            n++;
            i = Math.abs((index + n * n) % table.length);
        }
        if (fdi != -1) {
            table[fdi] = new QuadraticProbingMapEntry<>(key, value);
            size++;
            return null;
        } else if (table[i] == null || table[i].getKey().equals(key)) {
            table[i] = new QuadraticProbingMapEntry<>(key, value);
            size++;
            return null;
        } else {
            resizeBackingTable(2 * table.length + 1);
    ...
    public V remove(K key) {
        if (key == null) {
            throw new IllegalArgumentException("The key inputted is NULL!");
        }
    ...
    public void resizeBackingTable(int length) {
        ...
        if (length < size) {
            throw new IllegalArgumentException("The length is less than the number of items!");
        }
        QuadraticProbingMapEntry<K, V>[] tempo = table;
        table = (QuadraticProbingMapEntry<K, V>[]) new QuadraticProbingMapEntry[length];
        int z = 0;

        for (int i = 0; i < tempo.length; i++) {
            if (tempo[i] == null || tempo[i].isRemoved()) {
                continue;
            }


            int index = Math.abs(tempo[i].getKey().hashCode() % table.length);
            int k =  Math.abs(tempo[i].getKey().hashCode() % table.length);
            ...
            while (table[k] != null) {
                ...
                n++;
                k = Math.abs((index + n * n) % table.length);
